...
class Node(object):
    ...
    def receive_packet(self,packet):
        ...
        if packet.destination_address == 0:
            self.trace("%s received packet" % (self.hostname), packet.protocol)
        ...
        if packet.ttl <= 0:
            self.trace("%s dropping packet due to TTL expired" % (self.hostname), packet.protocol)
            return
    ...
    def forward_unicast_packet(self,packet):
        if packet.destination_address not in self.forwarding_table:
            self.trace("%s no routing entry for %d" % (self.hostname,packet.destination_address), packet.protocol)
            return
        link = self.forwarding_table[packet.destination_address]
        self.trace("%s forwarding packet to %d" % (self.hostname,packet.destination_address), packet.protocol)
        link.send_packet(packet)

    def forward_broadcast_packet(self,packet):
        for link in self.links:
            self.trace("%s forwarding broadcast packet to %s" % (self.hostname,link.endpoint.hostname), packet.protocol)
            packet_copy = copy.deepcopy(packet)
            link.send_packet(packet_copy)

if __name__ != "__main__":
    print "Imported lab5/node.py"
